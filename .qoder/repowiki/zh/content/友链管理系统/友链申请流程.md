...
## 更新摘要
**所做更改**
- 更新了友链申请界面的重构说明，从复杂的浮动按钮动画系统简化为单按钮直接打开申请表单
- 新增了右侧站长信息卡片的详细设计说明
- 更新了组件架构图以反映新的布局设计
- 完善了表单验证和状态管理的实现细节
...
**更新** 界面经过重大重构，从复杂的浮动按钮动画系统简化为单按钮直接打开申请表单，并增加了右侧站长信息卡片的全新设计，提升了用户体验和信息展示效率。
...
**章节来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L33-L41)
...
**图表来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L58-L70)
...
**更新** 组件经过重大重构，从复杂的浮动按钮动画系统简化为单按钮直接打开申请表单，并增加了右侧站长信息卡片的设计。
...
FriendLinkFab --> SiteInfoCard : 包含
ApplyFormValues --> ApplySchema : 符合
```

**图表来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L43-L70)
...
**章节来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L47-L56)

### 界面布局设计

**更新** 新的界面布局采用双栏设计，提供更直观的信息展示：

#### 主要布局结构

```mermaid
graph TB
Container[对话框容器] --> LeftPanel[左侧表单区域]
Container --> RightPanel[右侧站长信息卡片]
LeftPanel --> NameField[网站名称输入]
LeftPanel --> UrlField[网站地址输入]
LeftPanel --> LogoUpload[Logo上传]
LeftPanel --> DescriptionArea[签名描述]
LeftPanel --> EmailField[联系邮箱]
LeftPanel --> SubmitButton[提交按钮]
RightPanel --> AvatarSection[头像区域]
RightPanel --> InfoSection[基本信息]
RightPanel --> LinkInfo[友链信息]
AvatarSection --> AvatarImg[圆形头像]
AvatarSection --> OnlineBadge[在线状态徽章]
InfoSection --> UserName[用户名]
InfoSection --> Signature[签名]
InfoSection --> WebsiteLink[网站链接]
InfoSection --> LogoUrl[Logo地址]
```

**图表来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L83-L259)

#### 站长信息卡片设计

右侧站长信息卡片提供了完整的友链添加指导：
...
**章节来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L200-L256)
...
**章节来源**
- [src/app/(site)/friends/components/friend-link-fab.tsx](file://src/app/(site)/friends/components/friend-link-fab.tsx#L1-L263)
...
**更新** 界面重构显著提升了用户体验，单按钮设计简化了操作流程，右侧站长信息卡片提供了直观的信息展示，整体设计更加现代化和用户友好。
